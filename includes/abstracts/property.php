<?php
/**
 * PropertyManager product base class.
 *
 * @package PropertyManager\Abstracts
 */
 ...
 *
 * The PropertyManager product class handles individual product data.
 *
 * @version 3.0.0
 * @package PropertyManager\Abstracts
 */
class PM_Property extends PM_Data {
	 ...
	 *
	 * @param int|PM_Property|object $product Product to init.
	 */
	public function __construct( $product = 0 ) {
		...
		$this->data_store = PM_Data_Store::load( 'product-' . $this->get_type() );
		if ( $this->get_id() > 0 ) {
	 ...
	 * @since  3.0.0
	 * @param  string $context What the value is for. Valid values are view and edit.
	 * @return PM_DateTime|NULL object if the date is set or null if there is no date.
	 */
	...
	*/
	 ...
	 *
	 * @since  3.0.0
	 * @throws PM_Data_Exception Throws exception when invalid data is found.
	...
	protected function get_valid_tax_classes() {
		return PM_Tax::get_tax_class_slugs();
	}
	 ...
	 *
	 * @since 3.0.0
	 * @param array $raw_attributes Array of PM_Property_Attribute objects.
	 */
	public function set_attributes( $raw_attributes ) {
		$attributes = array_fill_keys( array_keys( $this->get_attributes( 'edit' ) ), null );
		foreach ( $raw_attributes as $attribute ) {
			if ( is_a( $attribute, 'PM_Property_Attribute' ) ) {
	 ...
	 *
	 * @since 3.0.0
	 * @param array $downloads_array Array of PM_Property_Download objects or arrays.
	 */
	public function set_downloads( $downloads_array ) {
		...
		foreach ( $downloads_array as $download ) {
			if ( is_a( $download, 'PM_Property_Download' ) ) {
				$download_object = $download;
			} else {
				$download_object = new PM_Property_Download();
	...
	*/
	...
	public function save() {
		 ...
		 *
		 * @param PM_Data          $this The object being saved.
		 * @param PM_Data_Store_WP $data_store THe data store persisting the data.
		 */
